...
@Component({
  selector: 'app-inversiones-proceso',
  templateUrl: './inversiones-proceso.component.html',
  styleUrls: ['./inversiones-proceso.component.css']
})
export class InversionesProcesoComponent implements OnInit {
    ...
    this._AuthService.obtenerInversionesproceso().subscribe((data:any) => {
      ...
      for (const iterator of this.inversiones) {
        if (iterator.usuarioUid == auth.uid) {
          ...
          this.arrayproceso.push(iterator)
        }
        
      }
       console.log("arrayproceso",this.arrayproceso);
      
    })// fin del getauth
    
    }); // fin del obeterninversionproceso
